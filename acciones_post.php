		...
		if ($con_bd->connect_error) {
			...
			if (!$sent_prep) {
				...
				if (!$sent_prep->execute()) {
					...
					$progreso_correcto = true;
				}
			}
			if (!$progreso_correcto || !$sent_prep_2) {
				$_SESSION['cod_comentario'] = 5;
			} else {
				$progreso_correcto = false;
				$sent_prep_2->bind_param("s", $id_steam);
				if (!$sent_prep_2->execute()) {
					$_SESSION['cod_comentario'] = 5;
				} else {
			...
			if ($progreso_correcto && (mysqli_num_rows($result_comentario) == 1 || mysqli_num_rows($result_usuario) == 1)) {
				// Enviar comentario a la BD.
